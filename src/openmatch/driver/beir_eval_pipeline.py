...
def main():
    ...
    post_process = "arguana" in encoding_args.output_dir
    ...
    #retriever = Retriever.build_embeddings(model, beir_dataset.query_datasets["test"], encoding_args)
    #Modified by HPX
    ...
    if encoding_args.local_process_index == 0:
        ...
        if post_process:
            q_flag_dict={}
            lines = []
            with open(encoding_args.trec_save_path, 'r') as rf:
                ...
                for i in range(len(new_lines)):
                    ...
                    if q==p:
                        ...
                        lines.append(" ".join([q, uu, p, str(int(r)-1), s, tt])+"\n")
            with open(encoding_args.trec_save_path, 'w') as fout:
                fout.writelines(lines)
            run = load_from_trec(encoding_args.trec_save_path)
        ...
        def print_line(measure, scope, value):
            subject = os.path.split(data_args.data_dir)[1]
            print('{:25s}{:8s}{:.4f}'.format(measure, scope, value))
            with open(os.path.join(os.path.split(encoding_args.output_dir)[0], "test_result.log"), "a", encoding="utf-8") as fw:
                fw.write('{:25s}{:25s}{:8s}{:.4f}\n'.format(subject, measure, scope, value))
