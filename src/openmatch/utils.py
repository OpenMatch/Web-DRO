...
#quoting=csv.QUOTE_NONE
    ...
    def read_qrel(relevance_file):
        ...
        with open(relevance_file, encoding='utf8') as f:
            ...
            for row in tsvreader:
                try:
                    topicid, _, docid, rel, clus = row
                except:
                    topicid, _, docid, rel = row
                    clus = None
                assert rel == "1"
                if topicid in qrel:
                    qrel[topicid].append(docid)
                    cluster_id[topicid].append(clus)
                else:
                    qrel[topicid] = [docid]
                    cluster_id[topicid] = [clus]
                
        return qrel, cluster_id
    ...
    def get_passage(self, p):
        ...
        if '__index_level_0__' in entry:
            new_entry = {}
            new_entry['title'] = entry['text_id']
            new_entry['text'] = entry['title']
            new_entry['url'] = entry['text']
            entry = new_entry
    ...
    def get_query_with_url(self, p): #this is only used for web-graph training
        entry = self.collection[int(p)] if p != "None" else self.collection[0]
        title = entry[self.title_field]
        title = "" if title is None else title
        body = entry[self.text_field]
        ...
        if self.doc_template is None:
            ...
            content = fill_template(self.doc_template, data=new_entry, allow_not_found=self.allow_not_found)
        # print(content)
        # exit(0)
        passage_encoded = self.tokenizer.encode(
            content,
    ...
    def get_passage_with_url(self, p):
        entry = self.collection[int(p)] if p != "None" else self.collection[0]
        title = entry[self.title_field]
        title = "" if title is None else title
        body = entry[self.text_field]
    ...
    def process_one_with_url(self, train):
        try:
            q, pp, nn, clus = train
            train_example = {
                'query': self.get_query_with_url(q),
                'positives': [self.get_passage_with_url(p) for p in pp],
                'negatives': [self.get_passage_with_url(n) for n in nn],
                'cluster_id': clus
            }
        except:
            q, pp, nn = train
