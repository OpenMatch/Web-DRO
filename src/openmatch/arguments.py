...
class ModelArguments:
    ...
    use_embedding_resize: bool = field(default=False, metadata={
                                     "help": "use embedding resize"})
...
class DRTrainingArguments(TrainingArguments):
    ...
    '''Modified by HPX: arguments for DRO'''
    use_dro: bool = field(default=False)
    train_alpha: bool = field(default=False) #True:训权重 False:训模型 
    reference_model_path: str = field(default="None")
    alpha_lr: float = field(default=1e-3)
    adjust_on_group_size: float = field(default=0) 
    alpha_log_interval: int = field(default=1)
    circular: bool = field(default=False) #严格从不同组顺次取样,对mixed_dataset无效
    alpha_path: str = field(default = "") #在train_alpha=False时使用 
    mixed_dataset: bool = field(default=False)
    n_clusters: int = field(default=None)
    distribution_counter: str = field(default = None)
    smooth_factor: float = field(default=0)
    momentum: float = field(default=0)
    training_steps: int = field(default=-1)
    truncate: bool = field(default=False)
    freeze_last_group: bool = field(default=False)
    wandb_id: str = field(default="")
    log_grad_norm: bool = field(default=False)
    log_emb_grad: bool = field(default=False)
