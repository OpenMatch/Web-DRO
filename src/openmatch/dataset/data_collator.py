...
@dataclass
class QPCollatorWithGroup(QPCollator):
    def __call__(self, features):
        q, d = super().__call__(features)
        #print(features[0].keys())
        if "cluster_id" in features[0]:
            key = "cluster_id"
        else:
            key = "cluster_id_"
        group_collated = [f[key] for f in features]
        return q, d, group_collated
