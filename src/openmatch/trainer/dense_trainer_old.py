...
class DRTrainer(Trainer):
    ...
    def _prepare_inputs(
        ...
        '''
        print(prepared)
        print(prepared[1]['input_ids'].shape)
        exit(0)
        '''
        return prepared
...
class DROTrainer(DRTrainer):
    def __init__(self, *args, **kwargs):
        ...
        self.alpha = torch.ones(self.n_clusters) / self.n_clusters
        self.alpha_lr = kwargs['args'].alpha_lr
        ...
        with open(self.alpha_save_path, "w"):
            pass
        '''
        self.train_weight = kwargs['args'].train_weight
        if self.train_weight:
            assert kwargs['args'].reference_model_path
            #psass
            #加载reference model
        else:
            assert not kwargs['args'].circular
        '''
    def compute_loss(self, model, inputs, return_outputs=False):
        '''
        if torch.distributed.get_rank() == 0:
            torch.save(inputs[0]['input_ids'], "/data1/hanpeixuan/q.pt")
            torch.save(inputs[1]['input_ids'], "/data1/hanpeixuan/d.pt")
        exit(0)
        '''
        query, passage, cluster_id = inputs
        
        assert all(x == cluster_id[0] for x in cluster_id)
        cluster_id = (int)(cluster_id[0])
    
        outputs = model(query=query, passage=passage)

        multiplier = torch.zeros(self.n_clusters)
        multiplier[cluster_id] = self.alpha_lr * (outputs.loss + self.adjust / self.group_cnt[(str)(cluster_id)])
        self.alpha = self.alpha * torch.exp(multiplier)
        self.alpha /= self.alpha.sum()
        
        outputs.loss *= self.alpha[cluster_id].item() * self.n_clusters #做了修改，让均值为1
        
        self.counter += 1
        if self.counter == self.interval:
            self.counter = 0
            with open(self.alpha_save_path, "a") as file:
                file.write(as_tsv(self.alpha))
        '''
        if torch.distributed.get_rank() == 0:
            print(cluster_id)
            print(outputs.loss)
